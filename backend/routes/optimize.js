...
let db;
//for deployment version: if db is not setup
try {
  db = await import("../db.js").then(mod => mod.db);
} catch (err) {
  console.warn("DB not available. History will not be saved.", err.message);
}
  ...
  try {
    ...
    if (db) {
      try {
        ...
        console.warn("Failed to save to DB:", dbErr.message);
      }
    }
